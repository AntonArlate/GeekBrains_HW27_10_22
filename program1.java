...
public class program1 {
    ...
    static void poleGenerator() { // создание двумерного поля из клеток заполненых 0. В дальнейшем 0 = стена
        ...
        for (int i = 0; i < xLenght; i++) {
            labyrinth.add(new ArrayList<Integer>(yLenght)); // первое измерение будет y [y1[x1,x2,x3],y2[x1,x2,x3]]
                                                            // arr.get[y].get[x]
        }
    ...
    static void test2() {
        ...
        for (int i = 0; i < x_axis_length; i++) {
            space.add(new ArrayList<Integer>(y_axis_length)); // добавляем строки необходимое число раз (теперь создаём
